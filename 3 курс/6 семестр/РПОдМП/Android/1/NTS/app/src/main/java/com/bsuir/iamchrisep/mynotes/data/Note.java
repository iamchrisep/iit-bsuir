...
public class Note {
    ...
    public static Note fromCursor(Cursor c) {
        ...
        note.setCreated(new DateTime(getLong(c, NotesContentContract.Notes.CREATED, 0), DateTimeZone.getDefault()));
        note.setUpdated(new DateTime(getLong(c, NotesContentContract.Notes.UPDATED, 0), DateTimeZone.getDefault()));
        note.setChecked(getBoolean(c,  NotesContentContract.Notes.CHECKED, false));
    ...
    private static Boolean getBoolean(Cursor c, String col, boolean defaultValue) {
        if (c.getColumnIndex(col) >= 0) {
            int a = c.getInt(c.getColumnIndex(col));
            boolean b = (a != 0);
            return b;
        } else {
            return defaultValue;
    ...
    public Note() {
        ...
        setCreated(DateTime.now(DateTimeZone.getDefault()));
        setUpdated(DateTime.now(DateTimeZone.getDefault()));
        setTitle("");
        setContent("");
        setChecked(false);
    }
    ...
    public ContentValues toContentValues() {
        ...
        values.put(NotesContentContract.Notes.CREATED, created.toDateTime(DateTimeZone.getDefault()).getMillis());
        values.put(NotesContentContract.Notes.UPDATED, updated.toDateTime(DateTimeZone.getDefault()).getMillis());
        values.put(NotesContentContract.Notes.CHECKED, checked);
